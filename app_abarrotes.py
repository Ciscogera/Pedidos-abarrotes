...
# Nuevas variables de entorno críticas anti-caídas
if "datos_borrador" not in st.session_state:
    st.session_state.datos_borrador = {}
if "pedido_calculado" not in st.session_state:
    st.session_state.pedido_calculado = False
if "df_pedido_data" not in st.session_state:
    st.session_state.df_pedido_data = None
...
def guardar_borrador_en_disco(responsable, fecha):
    """Resguarda en el disco duro del servidor el estado numérico actual digitado en el celular."""
...
def cargar_borrador_de_disco(responsable, fecha):
    """Rescata los datos guardados en caso de que el internet móvil haya parpadeado."""
    archivo = obtener_nombre_archivo_borrador(responsable, fecha)
...
if st.session_state.paso_flujo == "configuracion":
    ...
    if st.button(":material/expand_circle_down: INICIAR CONTEO DE STOCK", use_container_width=True):
        ...
        if not responsable_temp.strip():
            ...
            # 🔍 RESCATE: Si existe un borrador previo en el servidor para este usuario y fecha, lo inyectamos de golpe
...
# =====================================================================
    ...
    # Notificación sutil de recuperación de datos en el móvil
    if st.session_state.datos_borrador:
        st.info(":material/restore_page: **Borrador detectado y cargado.** Tu avance anterior ha sido restaurado en los casilleros.")

    # Fila de comandos superior para el pulgar
    ...
    with col_save:
        if st.button("💾 GUARDAR AVANCE", type="primary", use_container_width=True):
            ...
            st.toast("✅ ¡Avance guardado de forma segura en el servidor!", icon="💾")
    ...
    for cat in categorias:
        ...
        with st.expander(f":material/expand_circle_down: {cat} ({len(df_cat)} productos)"):
            for idx, row in df_cat.iterrows():
                ...
                # Inyección del backup si es que la señal se cortó a mitad del turno
    ...
    if st.button(":material/calculate: CALCULAR PEDIDO DE ABARROTES", use_container_width=True):
        ...
        st.session_state.df_pedido_data = pd.DataFrame(registros_pedido)
        
        if not st.session_state.df_pedido_data.empty:
            guardar_borrador_en_disco(st.session_state.responsable, st.session_state.fecha_inventario)
            ...
            st.session_state.pedido_calculado = True
            st.rerun()
        else:
            st.session_state.pedido_calculado = False
            st.success(" ¡Todos los niveles de stock están completos! No es necesario realizar pedidos hoy.")

    # Vista persistente: Asegura que los resultados no parpadeen ni desaparezcan al hacer clic
    if st.session_state.pedido_calculado and st.session_state.df_pedido_data is not None:
        df_pedido = st.session_state.df_pedido_data
